...
# Input cohort prefix
# read -p "${MAGENTA}Enter your cohort's prefix (e.g. hrr29):${COLORLESS} " COHORT_PREFIX
COHORT_PREFIX="hrr29"

if ! [[ $COHORT_PREFIX =~ $PREFIX_REGEX ]]; then
  echo "${RED}ERROR:${COLORLESS} Invalid prefix: ${COHORT_PREFIX}"
  echo "${YELLOW}Please enter a prefix that follows the convention hrrXX ${COLORLESS} (e.g. hrr29)"
  exit 1
fi
...
# Input GitHub handle
read -p "${MAGENTA}Enter your GitHub handle EXACTLY as it's spelled and capitalized on GitHub:${COLORLESS} " GITHUB_HANDLE
